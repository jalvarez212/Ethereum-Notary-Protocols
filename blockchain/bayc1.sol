...
library Address {
    ...
    function sendValue(address payable recipient, uint256 amount) internal {
        require(address(recipient).balance >= amount, "Address: insufficient balance");
...
contract Nft is ERC721, Ownable {
    ...
    function mintNft(uint numberOfTokens) public payable {
        ...
        require(totalSupply().add(numberOfTokens) <= MAX_NFT, "Purchase would exceed max supply of Nft");
...
contract IAccountFactory{
    ...
    function createINft(string memory name, string memory symbol, uint256 maxNftSupply) public returns (Nft) {
        newNft = new Nft(name, symbol, maxNftSupply);
        return newNft;
